...
Moderne Entwicklungsumgebungen können uns dabei die
Arbeit erleichtern. Sie bieten vielfach die Möglichkeit den
Quelltext automatisch zu formatieren.
...
Daten besitzen einen \textbf{Wert}  -- eine konkrete Zahl oder einen Buchstaben -- 
...
Zeichenketten wie {\tt ''Hello, world!''}, die auf dem Bildschirm ausgegeben wurden.  
...
Daten repräsentieren unterschiedliche Dinge wie zum Beispiel die 
ganzen Zahlen, die reellen Zahlen und Buchstaben. Wir unterscheiden die Daten
anhand ihres \textbf{Typs}.

Es ist wichtig, dass ein Programm ganz genau weiß, um welche 
Art von Daten es sich bei den gespeicherten Werten handelt, 
denn nicht alle Manipulationen sind für jeden Datentyp sinnvoll.
Wir können zwei Zahlen addieren, aber die Addition von \texttt{'a' + 'a'}
ist nicht definiert.

Ein weiterer Grund für die Unterscheidung der Datentypen liegt in
den unterschiedlichen Anforderungen für die Speicherung der Daten
im Computer. Daten werden als Bitfolgen gespeichert. Ein Bit ist die  
kleinste mögliche Informationseinheit.
\index{Bit}

Für die Speicherung von \emph{Buchstaben} reichen üblicherweise 7 oder 8 Bit.
Ein einelnes Bit kann eine einfache Fallunterscheidung treffen 0 oder 1, an oder aus. Mit 8 Bit können wir
256 Fälle unterscheiden -- mehr als genug für die Zeichen des Alphabets,
der Ziffern und Satzzeichen. Für die Speicherung von 
\emph{Zahlen} werden aber wesentlich mehr Bit benötigt. Wir wollen ja nicht nur
von 0 bis 255 zählen können. 

Der Datentyp legt also fest, welche Bedeutung diese Bitfolgen haben: \emph{Buchstabe}, \emph{ganze Zahlen}, usw. 
...
Computer speichern Daten in Gruppen einzelner
Bits. Eine Gruppe von 8 Bit nennt man Byte und jedes Byte hat im Computer eine 
...
\index{Byte}
\index{Adresse}
Als Programmierer einer Hochsprache muss man die genaue numerische Adresse
der Daten nicht kennen. Da wir uns als Menschen besser Namen 
als lange Zahlenfolgen merken, werden wir im nächsten Abschnitt kennenlernen wie 
wir \emph{Variablen} benutzen um Daten zu speichern und zu bearbeiten.


Die \emph{ganzen Zahlen} (beispielsweise 1 oder 17) werden in C als \emph{integer} 
bezeichnet.  Wir können nicht nur Zeichenketten sondern auch 
ganze Zahlen auf dem Bildschirm ausgeben
lassen. Die Ausgabe sieht auf den ersten Blick nicht anders aus, wenn wir uns
eine Zahl ausgeben lassen oder eine Zeichenkette.
...
Wenn wir uns die  \texttt{printf()} Anweisung genau anschauen, fällt
zwischen den Anführungszeichen ein \texttt{\%i} auf. Das ist ein 
Platzhalterzeichen, welches angibt das eine ganze Zahl ausgegeben werden
soll. Es gibt eine Reihe solcher Platzhalter für unterschiedliche Datentypen.
...
Wir können Daten vom Typ \emph{character} auch ausgeben:
...
\vskip 0.5em
%
Dafür benötigen wir das Platzhalterzeichen \texttt{\%c}.
...
ist die Möglichkeit, Werte durch die Verwendung von {\bf Variablen} zu
manipulieren.  
In unseren bisherigen Versuchen waren alle Werte unserer Anweisungen
durch die Angaben im Quelltext des Programms statisch festgelegt. 
Ab jetzt werden wir oft Variablen benutzen um Werte zu speichern und zu
verändern. Variablen könne wir uns wie die Speicherfunktion an
einem Taschenrechner vorstellen, nur viel flexibler und mächtiger, weil wir 
beliebig viele Variablen in unserem Programm anlegen können.  

Da die Variablen verschiedene Werte unserer Daten speichern sollen,
müssen auch die Variablen verschiedene Typen unterstützten.
Es gibt einige Programmiersprachen, bei denen der Computer 
selbstständig den Typ der Variable anhand des zu speichernden
Werts erkennt. In C müssen wir den Typ immer selbst angeben.
Wenn wir eine neue Variable erzeugen, müssen wir deren Typ deklarieren.
So wird der Typ \emph{character} in C als {\tt char} angegeben.  
Die folgende Anweisung erzeugt eine neue Variable 
...
Zahlen können  hingegen in  {\tt int} Variablen gespeichert werden.


\index{Deklaration}
\index{Anweisung!Deklaration}

Um eine einfache \emph {integer} Variable anzulegen, verwenden wir folgende
Syntax:
...
Idee dabei Namen zu wählen, welche die Daten beschreiben,
die in ihnen gespeichert werden sollen. Das erleichtert uns den
Umgang mit einer Vielzahl von Variablen und macht ein Programm
leichter lesbar.
...
sind beides ganze Zahlen ({\tt int} Typ).
